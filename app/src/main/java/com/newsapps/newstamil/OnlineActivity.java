...
import com.newsapps.newstamil.offline.OfflineActivity;
...
import com.newsapps.newstamil.types.Feed;
import com.newsapps.newstamil.types.Label;
...
//		SharedPreferences localPrefs = getSharedPreferences("localprefs", Context.MODE_PRIVATE);
		...
		SharedPreferences localPrefs = getSharedPreferences("localprefs", MODE_PRIVATE);
	...
	public void login(boolean refresh, OnLoginFinishedListener listener) {
		if (m_prefs.getString("server_url", "").trim().length() == 0) {
			...
			builder.setMessage(R.string.dialog_need_configure_prompt)
			       .setCancelable(false)
			       .setPositiveButton(R.string.dialog_open_preferences, new OnClickListener() {
	...
	public void checkTrial(boolean notify) {
        if (!BuildConfig.DEBUG) {
            ...
            boolean isTrial = getPackageManager().checkSignatures(
                    getPackageName(), "com.newsapps.newstamil") != PackageManager.SIGNATURE_MATCH;
	...
	private void openUnlockUrl() {
		try {
			Intent intent = new Intent(Intent.ACTION_VIEW, 
				Uri.parse("market://details?id=com.newsapps.newstamil"));
			startActivity(intent);
		} catch (ActivityNotFoundException ae) {
			try {
				Intent intent = new Intent(Intent.ACTION_VIEW, 
					Uri.parse("https://play.google.com/store/apps/details?id=com.newsapps.newstamil"));
				startActivity(intent);
			} catch (Exception e) {
				e.printStackTrace();
	...
	public void editArticleNote(final Article article) {
		...
		builder.setPositiveButton(R.string.article_set_note, new OnClickListener() {
	        public void onClick(DialogInterface dialog, int which) {
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		for (PackageInfo p : pkgs) {
			if ("com.newsapps.newstamil".equals(p.packageName)) {
	...
	protected void initMenu() {
		if (m_menu != null) {			
			...
			*/
			...
			/*
			m_menu.findItem(R.id.set_labels).setEnabled(false).setVisible(false);
			m_menu.findItem(R.id.article_set_note).setEnabled(false).setVisible(false);
			m_menu.findItem(R.id.subscribe_to_feed).setEnabled(false).setVisible(false);
			*/
			...
			if (ap != null) {
				...
				if (article != null) {
					...
					/*m_menu.findItem(R.id.toggle_published).setIcon(article.published ? R.drawable.ic_checkbox_marked :
							R.drawable.ic_rss_box).setEnabled(false).setVisible(false);*/
							...
							ApiRequest req = new ApiRequest(m_context) {
								protected void onPostExecute(JsonElement result) {
									...
									if (result != null) {
										...
										// Unknown method means old , in that case we assume API 0 and continue
